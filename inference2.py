...
def extract_radiomics(image_path, mask_img):
    ...
    extractor.enableAllFeatures()  # Enable all to match training likelihood
    ...
    try:
        ...
        # Return zeros array to prevent crash
        # Expected total 1419 - 1024 (Deep) = 395 Radiomics Features
        return np.zeros(395)

# Load Models (Global Scope)
dl_model = load_dl_model("model_densenet121_deeplearning.pth")
bundle = joblib.load("model_lgbm_machinelearning.joblib")
...
def preprocess_image_median_clahe(image_bytes):
    ...
    # Convert back to RGB for DenseNet model
...
def run_cli_inference(image_path):
    ...
    # Use the median+clahe preprocess for the tensor passed to model
    img_tensor = preprocess_image_median_clahe(contents)
    ...
    # Resize heatmap to match original image size for Radiomics extraction
    # Note: Using img_cv.shape to match original image dimensions
    ...
    try:
        ...
        result = {
            ...
            "prediction": {
                ...
                "model_used": "DenseNet121 + LGBM Hybrid (Median+CLAHE+Radiomics)"
            },
        ...
        print(f"[INFO] Grad-CAM saved to: {output_img_path}")
        ...
        print(json.dumps({
            "status": "error", 
            "message": str(e),
            "hint": f"Expected {scaler.n_features_in_} features, got {input_features.shape[1]}."
        }, indent=4))
    except Exception as e:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python inference2.py <image_path>")
        sys.exit(1)
